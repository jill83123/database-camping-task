...
-- 5-2. 修改：`王小明` 取消預約 `李燕容` 的課程，請在 `COURSE_BOOKING` 更新該筆預約資料：
    -- 1. 取消預約時間 `cancelled_at` 設為 2024-11-24 17:00:00
...
-- 5-3. 新增：`王小明` 再次預約 `李燕容` 的課程，請在 `COURSE_BOOKING` 新增一筆資料：
    -- 1. 預約人設為 `王小明`
    -- 2. 預約時間 `booking_at` 設為 2024-11-24 17:10:25
    -- 3. 狀態 `status` 設定為即將授課
...
SELECT
  ...
  (
    "CREDIT_PURCHASE".total_credit - "COURSE_BOOKING".used_credit
  ) AS remaining_credit
FROM (
    SELECT
      user_id,
...
FROM
  "COURSE_BOOKING"
WHERE
  created_at BETWEEN '2024-12-01' AND '2024-12-31'
  AND
  status != '課程已取消';
